...
- Auth via Supabase LinkedIn OIDC
...
- Supabase tables (requires own Supabase project): `user_profiles`, `leads`, `intros`, `qualification_configs` — all with RLS
- Deployed to Vercel
...
Full task breakdown with checkable milestones lives in `todo.md`. All 8 milestones complete. Project is archived (EOL).
